...
void sgxErrorHandler(int code) {
  ...
  if (code == -2)
    ereport(ERROR,
            (-1, errmsg("SGX_ERROR_CODE %d: ENCLAVE IS NOT RUNNING", code)));
  if (code == -3)
  ...
  if (code == -5)
    ereport(ERROR,
            (-1, errmsg("SGX_ERROR_CODE %d: MEMORY_ALLOCATION_ERROR", code)));
  if (code == -6)
    ereport(ERROR,
            (-1, errmsg("SGX_ERROR_CODE %d: OUT_OF_THE_RANGE_ERROR", code)));
  if (code == -7)
    ereport(ERROR,
            (-1, errmsg("INTERFACE_ERROR_CODE %d: BASE64DECODER_ERROR", code)));
  if (code == -8)
    ereport(ERROR, (-1, errmsg("INTERFACE_ERROR_CODE %d: \n The extension was "
                               "not initialized. Run 'select lau   nch();'",
                               code)));
  if (code == -9)
    ereport(
        ERROR,
        (-1,
         errmsg("INTERFACE_ERROR_CODE %d: \n Cannot open key storage file.'",
                code)));
  if (code == -10)
    ereport(ERROR,
            (-1, errmsg("INTERFACE_ERROR_CODE %d: \n The default master key "
                        "was not set up. Run 'select generate_key();'.",
                        code)));
  if (code == TOO_MANY_ELEMENTS_IN_BULK)
    ereport(ERROR, (-1, errmsg("INTERFACE_ERROR_CODE %d: Too many elements in "
                               "a bulk for an aggregation function.",
                               code)));
}

// no actual use & conflict with cstore_fdw ones;
...
//  ereport(
//      INFO,
//      (errmsg("StealthDB is initialized: the default key has been loaded.")));
//}
//
//PG_FUNCTION_INFO_V1(_PG_fini);
//Datum _PG_fini() {
//  printf("Called _PG_fini function but nothing happens here!");
//}
...
Datum generate_key(PG_FUNCTION_ARGS) {
...
Datum load_key(PG_FUNCTION_ARGS) {
