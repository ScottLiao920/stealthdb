...
int enc_int32_sum_bulk(size_t bulk_size, char* arg1, char* res) {
  ...
  while (counter < bulk_size) {
    if (!FromBase64Fast((const BYTE*)arg1 + arg_position,
                        ENC_INT32_LENGTH_B64 - 1, int2_v, ENC_INT32_LENGTH))
  ...
  while (true) {
    if (req->is_done == -1) {
      ...
      resp = req->resp;
      if (!ToBase64Fast((const BYTE*)int3_v, ENC_INT32_LENGTH, res,
...
int enc_int32_ops(int cmd, char* int1, char* int2, char* res) {
  ...
  std::array<BYTE, ENC_INT32_LENGTH> int1_v; //array of ENC_INT32_LENGTH bytes
  std::array<BYTE, ENC_INT32_LENGTH> int2_v;
  std::array<BYTE, ENC_INT32_LENGTH> int3_v;

  // read byte from int1 to int1_v
  if (!FromBase64Fast((const BYTE*)int1, ENC_INT32_LENGTH_B64 - 1,
                      int1_v.begin(), ENC_INT32_LENGTH))
    return BASE64DECODER_ERROR;

  // read byte from int2 to int2_v
  if (!FromBase64Fast((const BYTE*)int2, ENC_INT32_LENGTH_B64 - 1,
                      int2_v.begin(), ENC_INT32_LENGTH))
    return BASE64DECODER_ERROR;
  ...
  while (true) {
    if (req->is_done == -1) {
      __asm__("pause");  // used to improved the performance; inform cpu that
                         // this program is executing an iterative waiting
                         // process; prevent memory order violation in most
                         // cases leading to performance boost;
    } else
    /*TODO: findout when req->is_done get updated other than here*/
    // current thought: enclave process in enclave.cpp iteratively processes
    // requests & set req.is_done to 0, hence in this case request is already
    // processed
    {
      // get the result from request.buffer
      std::copy(&req->buffer[2 * ENC_INT32_LENGTH],
                &req->buffer[3 * ENC_INT32_LENGTH], int3_v.begin());

      resp = req->resp; // update status
      if (!ToBase64Fast((const BYTE*)int3_v.begin(), ENC_INT32_LENGTH, res,
                        ENC_INT32_LENGTH_B64)) // decode from raw to int
        resp = BASE64DECODER_ERROR;
      spin_unlock(&req->is_done);
      break;
    }
...
int enc_int32_cmp(char* int1, char* int2, char* res) {
  ...
  while (true) {
    if (req->is_done == -1) {
      ...
      resp = req->resp;
      std::copy(&req->buffer[2 * ENC_INT32_LENGTH],
                &req->buffer[2 * ENC_INT32_LENGTH + INT32_LENGTH], &res[0]);
      spin_unlock(&req->is_done);
      break;
    }
  }
...
int enc_int32_encrypt(int pSrc, char* pDst) {
  ...
  if (!ToBase64Fast((const BYTE*)int1_v.begin(), ENC_INT32_LENGTH, pDst,
                    ENC_INT32_LENGTH_B64))
    resp = BASE64DECODER_ERROR;
...
int enc_int32_decrypt(char* pSrc, char* pDst) {
  ...
  std::array<BYTE, ENC_INT32_LENGTH> int1_v;
  if (!FromBase64Fast((const BYTE*)pSrc, ENC_INT32_LENGTH_B64 - 1,
                      int1_v.begin(), ENC_INT32_LENGTH))
    return BASE64DECODER_ERROR;
  ...
  while (true) {
    if (req->is_done == -1) {
      ...
      std::copy(&req->buffer[ENC_INT32_LENGTH],
                &req->buffer[ENC_INT32_LENGTH + INT32_LENGTH], &pDst[0]);
      resp = req->resp;
      spin_unlock(&req->is_done);
      break;
    }
