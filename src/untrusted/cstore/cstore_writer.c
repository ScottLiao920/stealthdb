...
#include "cstore/cstore_metadata_serialization.h"
...
SerializeBlockData(TableWriteState *writeState, uint32 blockIndex, uint32 rowCount) {
    ...
    for (columnIndex = 0; columnIndex < columnCount; columnIndex++) {
        ...
        if (compressed) {
            // TODO: check data type, if it's an encrypted type, then encrypt them (completed)
            // fixme: currently reader cannot work
