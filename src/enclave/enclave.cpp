...
int generateKeyEnclave(uint8_t *sealed_key, size_t sealedkey_len) {
    ...
    resp = sgx_seal_data(0, NULL, len, p_key_tmp, sealedkey_len,
...
int loadKeyEnclave(uint8_t *sealed_key, size_t sealedkey_len) {
    ...
    if (p_key == NULL) p_key = new sgx_aes_ctr_128bit_key_t[len];

    if (sgx_calc_sealed_data_size(0, sizeof(sgx_aes_ctr_128bit_key_t)) >
        sealedkey_len)
        return MEMORY_COPY_ERROR;

    resp = sgx_unseal_data((const sgx_sealed_data_t *) sealed_key, NULL, NULL,
                           (uint8_t *) p_key, &len);
...
/* Decrypts byte array by aesgcm mode
 ...
 @return:
    * SGX_error, if there was an error during encryption/decryption
    0, otherwise
*/
int decrypt_bytes(uint8_t *pSrc, size_t src_len, uint8_t *pDst,
                  size_t dst_len) {
    int resp = sgx_rijndael128GCM_decrypt(
            ...
            NULL,
            0,  // aad
            (sgx_aes_gcm_128bit_tag_t *) (pSrc - SGX_AESGCM_MAC_SIZE +
                                          src_len));  // tag
...
int encrypt_bytes(uint8_t *pSrc, size_t src_len, uint8_t *pDst,
    ...
    if (resp != SGX_SUCCESS) return resp;

    memcpy(pDst, nonce, SGX_AESGCM_IV_SIZE);
    resp = sgx_rijndael128GCM_encrypt(
            p_key, // pointer to key
            pSrc, // pointer to plain text to be encrypted
    ...
    while (true) {
        ...
        if (req == NULL)
            ...
            switch (req->ocall_index) {
                case CMD_INT64_PLUS:
                    req->resp =
                            enc_int32_add(req->buffer, ENC_INT32_LENGTH,
                ...
                case CMD_INT64_MINUS:
                    req->resp =
                            enc_int32_sub(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH + ENC_INT32_LENGTH,
                                          ENC_INT32_LENGTH);
                    break;
                ...
                case CMD_INT64_MULT:
                    req->resp =
                            enc_int32_mult(req->buffer, ENC_INT32_LENGTH,
                                           req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                                           req->buffer + ENC_INT32_LENGTH + ENC_INT32_LENGTH,
                                           ENC_INT32_LENGTH);
                    break;
                ...
                case CMD_INT64_DIV:
                    req->resp =
                            enc_int32_div(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH + ENC_INT32_LENGTH,
                                          ENC_INT32_LENGTH);
                    break;
                ...
                case CMD_INT64_EXP:
                    req->resp =
                            enc_int32_pow(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH + ENC_INT32_LENGTH,
                                          ENC_INT32_LENGTH);
                    break;
                ...
                case CMD_INT64_MOD:
                    req->resp =
                            enc_int32_mod(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH + ENC_INT32_LENGTH,
                                          ENC_INT32_LENGTH);
                    break;
                ...
                case CMD_INT64_CMP:
                    req->resp =
                            enc_int32_cmp(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, ENC_INT32_LENGTH,
                ...
                case CMD_INT32_SUM_BULK:
                    ...
                    req->resp = enc_int32_sum_bulk(
                            req->buffer, INT32_LENGTH, req->buffer + INT32_LENGTH,
                ...
                case CMD_INT64_ENC:
                    req->resp =
                            encrypt_bytes(req->buffer, INT32_LENGTH,
                                          req->buffer + INT32_LENGTH, ENC_INT32_LENGTH);
                    break;

                case CMD_INT64_DEC:
                    req->resp =
                            decrypt_bytes(req->buffer, ENC_INT32_LENGTH,
                                          req->buffer + ENC_INT32_LENGTH, INT32_LENGTH);
                    break;

                case CMD_FLOAT4_PLUS:
                    req->resp = enc_float32_add(
                            req->buffer, ENC_FLOAT4_LENGTH, req->buffer + ENC_FLOAT4_LENGTH,
                            ENC_FLOAT4_LENGTH, req->buffer + 2 * ENC_FLOAT4_LENGTH,
                            ENC_FLOAT4_LENGTH);
                    break;

                case CMD_FLOAT4_MINUS:
                    req->resp = enc_float32_sub(
                            req->buffer, ENC_FLOAT4_LENGTH, req->buffer + ENC_FLOAT4_LENGTH,
                            ENC_FLOAT4_LENGTH, req->buffer + 2 * ENC_FLOAT4_LENGTH,
                            ENC_FLOAT4_LENGTH);
                    break;
                ...
                case CMD_FLOAT4_ENC:
                    req->resp =
                            encrypt_bytes(req->buffer, FLOAT4_LENGTH,
                                          req->buffer + FLOAT4_LENGTH, ENC_FLOAT4_LENGTH);
                    break;

                case CMD_FLOAT4_DEC:
                    req->resp =
                            decrypt_bytes(req->buffer, ENC_FLOAT4_LENGTH,
                ...
                case CMD_STRING_CMP:
                    ...
                    memcpy(src2, req->buffer + INT32_LENGTH + src_len + INT32_LENGTH,
                           src2_len);
                    req->resp =
                            enc_text_cmp(src1, src_len, src2, src2_len,
                ...
                case CMD_STRING_LIKE:
                    ...
                    req->resp = enc_text_like(src1, src_len, src2, src2_len,
                                              req->buffer + buf_pos, INT32_LENGTH);
                    buf_pos = 0;
                    break;
                ...
                case CMD_STRING_SUBSTRING:
                    ...
                    req->resp = enc_text_substring(src1, src_len, in1, src2_len, in2,
                                                   src3_len, dst, &dst_len);
                ...
                case CMD_STRING_CONCAT:
                    ...
                    dst_len =
                            src_len + src2_len - SGX_AESGCM_IV_SIZE - SGX_AESGCM_MAC_SIZE;

                    req->resp =
                            enc_text_concatenate(src1, src_len, src2, src2_len, dst, dst_len);
                    memcpy(req->buffer + buf_pos, &dst_len, INT32_LENGTH);
                    buf_pos += INT32_LENGTH;
                ...
                case CMD_TIMESTAMP_EXTRACT_YEAR:
                    req->resp = enc_timestamp_extract_year(
                            req->buffer, ENC_TIMESTAMP_LENGTH,
                            req->buffer + ENC_TIMESTAMP_LENGTH, ENC_INT32_LENGTH);
                    break;

                case CMD_TIMESTAMP_CMP:
                    req->resp = enc_timestamp_cmp(
                            req->buffer, ENC_TIMESTAMP_LENGTH,
                            req->buffer + ENC_TIMESTAMP_LENGTH, ENC_TIMESTAMP_LENGTH,
                            req->buffer + 2 * ENC_TIMESTAMP_LENGTH, INT32_LENGTH);
                    break;

                case CMD_TIMESTAMP_ENC:
                    req->resp = encrypt_bytes(req->buffer, TIMESTAMP_LENGTH,
                                              req->buffer + TIMESTAMP_LENGTH,
                                              ENC_TIMESTAMP_LENGTH);
                    break;

                case CMD_TIMESTAMP_DEC:
                    req->resp = decrypt_bytes(req->buffer, ENC_TIMESTAMP_LENGTH,
                                              req->buffer + ENC_TIMESTAMP_LENGTH,
                                              TIMESTAMP_LENGTH);
                    break;
